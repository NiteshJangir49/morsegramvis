...
class Viewparticles(Window):
    ...
    def __init__(self, parent=None):
        ...
        self.coord_nums = {}
        self.coord_range = (0, 12)   # default; overridden below if file found
        # if utils.check_file(Config.COORD_NUM_FILE):
        self.coord_nums = utils.load_coordination_numbers("dataset_files/Steel Beads/data/cn_file.txt")
        if self.coord_nums:
            mn = min(self.coord_nums.values())
            mx = max(self.coord_nums.values())
            self.coord_range = (mn, mx)
            print(f"[coordination] range: {mn} – {mx}")
        # =======================================================
        ...
        self.update_actors(self.iren, utils.get_actors_list(self.maxima_data, self.contact_points,
                                                            grain_id, self.ren, Config.PARTICLES_MESH_DIR + str(grain_id) + ".vtp", coord_nums=self.coord_nums,
    ...
    def onToolbarViewAction(self, s):
        ...
        if self.view_type == utils.ViewType.NEIGHBOR:
                ...
                for contact_pt in self.contact_points[grain_id]:
                    if self.neighbor[contact_pt.sibling_cp_id][0]:
                        actors_list += utils.get_actors_list(self.maxima_data, self.contact_points, contact_pt.sibling_cp_id, self.ren, 
                                                             Config.PARTICLES_MESH_DIR + str(contact_pt.sibling_cp_id) + ".vtp", False, coord_nums=self.coord_nums,
                                                    coord_range=self.coord_range)
            except KeyError:
            ...
            actors_list += utils.get_actors_list(self.maxima_data, self.contact_points, grain_id, self.ren,
                                                  Config.PARTICLES_MESH_DIR + str(grain_id) + ".vtp", coord_nums=self.coord_nums,
                                                    coord_range=self.coord_range)
        ...
        for node in nodes:
            try:
                actors += utils.get_actors_list(self.maxima_data, self.contact_points, node, self.ren, 
                                                Config.PARTICLES_MESH_DIR + str(node) + ".vtp", isContacts=False, coord_nums=self.coord_nums,
                                                    coord_range=self.coord_range)
